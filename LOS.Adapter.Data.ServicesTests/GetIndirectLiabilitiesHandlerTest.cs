...
{
    [TestClass()]
    public class GetIndirectLiabilitiesHandlerTest
    {
        ...
        [TestMethod]
        public void GetIndirectLiabilitiesHandlerTests()
        {
            string user = "UGRM175";
            string pass = "*";
            string bankId = "80";
            ...
            string itemNumber = "";
            string luw = Constants.GET_INDIR_LUW_ID;
            string ObligationNumber = "0000000042";

            var reply = requestBus.ExecuteAsync(new GetIndirectLiabilitiesRequest(user, pass, bankId, ObligorNumber, "", ObligationNumber, "", "", "", itemNumber, luw, ""));
            reply.GetAwaiter().GetResult();

            var respList = reply.Result.IndirectLiabilityDetails as List<IndirectLiabilities>;
            Assert.IsNotNull(respList);
            Assert.IsTrue(respList.Count > 0);
            respList.RemoveNullOrEmpty<IndirectLiabilities>();
            Assert.IsTrue(respList.Count > 0 && respList.Count < 200);
