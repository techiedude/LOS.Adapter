...
{
    [TestClass()]
    public class GetBillingSchedulesHandlerTests
    {
        ...
        [TestMethod]
        public void GetBillingSchedulesHandlerTest()
        {
            string user = "UGRM175";
            string pass = "*";
            string bankId = "80";
            ...
            string itemNumber = "";
            string luw = Constants.GET_BILL_SCH_LUW_ID;
            string ObligationNumber = "0000000042";

            var reply = requestBus.ExecuteAsync(new GetBillingSchedulesRequest(user, pass, bankId, ObligorNumber, "", ObligationNumber, "", "", "", itemNumber, luw, ""));
            reply.GetAwaiter().GetResult();

            var respList = reply.Result.billingSchedulesDetails as List<BillingSchedulesDetails>;
            Assert.IsNotNull(respList);
            Assert.IsTrue(respList.Count > 0);
            respList.RemoveNullOrEmpty<BillingSchedulesDetails>();
            Assert.IsTrue(respList.Count > 0 && respList.Count < 142);
